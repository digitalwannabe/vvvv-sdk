...
namespace VVVV.Nodes
{
    ...
    #region PluginInfo
    [PluginInfo(Name = "AmazonS3Upload",
                Category = "Network",
                Help = "Upload to S3",
    ...
    public class AmazonS3UploadNode : IPluginEvaluate, IPartImportsSatisfiedNotification, IDisposable
    {
        ...
        public void Evaluate(int SpreadMax)
        {
            if (FPinInDoSend[0])
            {
                SpreadMax = SpreadUtils.SpreadMax(FPinInDoSend, FPinInPath, FPinInBucket, FPinInSubdir);
 ...
 //                       FStringOut[i] = string.Empty;
                        ...
                        var task = Task.Factory.StartNew(() =>
                        {
                            ...
                            string fileToBackup = FPinInPath[i]; // test file
                            string myBucketName = FPinInBucket[i]; //your s3 bucket name goes here
                            string s3DirectoryName = FPinInSubdir[i];
                            string s3FileName = System.IO.Path.GetFileName(FPinInPath[i]);

                            var result = sendMyFileToS3(fileToBackup, myBucketName, s3DirectoryName, s3FileName);
